{include file='header.tpl' p="article" rss="article-category"} 
<div id="wrapper">
    <div class="container-fluid">
      <div class="row-fluid">
        ...
        {else}
            ...
            <span class="pull-right"><a href="{$smarty.const._URL}/admin/article_manager.php?do=edit&id={$article.id}" title="{$lang.edit}" target="_blank" class="btn btn-mini">{$lang.edit}</a></span>
            {/if}
            ...
            {$lang.articles_by} <a href="{$smarty.const._URL}/profile.{$smarty.const._FEXT}?u={$article.username}">{$article.name}</a> / <strong>{$article.views_formatted}</strong> {$lang.views}
            </div>

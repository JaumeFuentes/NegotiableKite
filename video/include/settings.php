...
$link = new Links;
$listaLinks = $link->generaListaLinks();

//--------------------------------------------------------------------------------------------//
error_reporting(E_ALL & ~E_NOTICE &  ~E_STRICT); // Development value: E_ALL & ~E_NOTICE 
...
define('_YOUTUBETHUB', 'http://img.youtube.com/vi'); // CURRENT YOUTUBE URL FOR THUMBS
...
define('_THUMB_FROM', $config['thumb_from']); // FETCH THUMBS FROM YOUTUBE OR LOCALHOST ? (1 = Youtube.com // 2 = Your server)
define('_BROWSER_PAGE', $config['browse_page']); // Number of results per category page
define('_ISNEW_DAYS', $config['isnew_days']); // How many days should a video stay marked as 'NEW'.
define('_ISPOPULAR', $config['ispopular']); // Define the minimum number of views a video needs to become tagged as POPULAR
define('_STOPBADCOMMENTS', $config['stopbadcomments']); // Don't post comments that contain bad words: bad_words.txt
define('_HTMLCOUNTER', stripslashes($config['counterhtml']));
define('_FAV_LIMIT', stripslashes($config['fav_limit'])); // Favorite videos limit / user

define('_PM_VERSION', $config['version']); // PHP MELODY VERSION
define('_TPLFOLDER', $config['template_f']); // CURRENT YOUTUBE URL FOR THUMBS
define('_SEOMOD', $config['seomod']);	// SHOW SEO FRIENDLY URLS OR NOT
define('_MOD_ARTICLE', $config['mod_article']);
define('_ALLOW_USER_UPLOADVIDEO', $config['allow_user_uploadvideo']);
define('_ALLOW_USER_SUGGESTVIDEO', $config['allow_user_suggestvideo']);
define('_MOD_SOCIAL', $config['mod_social']);
...
define('_ISSMTP', $config['issmtp']);

define('_USE_HQ_VIDS', $config['use_hq_vids']);
...
define('_TOPVIDS', $config['top_videos']);
define('_NEWVIDS', $config['new_videos']);
...
$smarty->register_function('dropdown_menu_video_categories', 'list_categories');
...
//added by NK
$smarty->assign("listaLinks", $listaLinks);

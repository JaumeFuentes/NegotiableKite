...
function request_video($unique_id = '', $page = "detail", $autoplay = false)
{
	...
	if ($video['video_player'] == 'embed')
	{
		...
		$embed_code = str_replace("%%yt_thumb%%", $video['preview_image'], $embed_code);
	...
	$video['video_href'] = makevideolink($video['uniq_id'], $video['video_title']);
	$video['thumb_img_url'] = show_thumb($video['uniq_id']);
...
function get_video_list($orderby = '', $sort = '', $start = 0, $limit = 5, $category_id = 0, $video_ids = array(), $uniq_ids = array()) 
{
	...
	while ($row = mysql_fetch_assoc($result))
	{
		...
		$list[$i]['video_href'] = makevideolink($row['uniq_id'], $row['video_title']);
		$list[$i]['thumb_img_url'] = show_thumb($row['uniq_id']);
...
function get_related_video_list($category_id = 0, $video_title = '', $limit = 5)
{
	global $config;
...
function get_top_rated_video_list($category_id = 0, $limit = 5)
{
	if ( ! $category)
	{
...
function generate_embed_code($uniq_id = '', $video = array(), $append_backlink = true)
{
	$embed_code = '';
	$backlink 	= '';
	...
	$embed_code .= '>';
	$embed_code .= '<param name="scale" value="noscale" />';
	...
	if ($append_backlink)
	{
		$backlink = '<p><a href="'. makevideolink($video['uniq_id'], $video['video_title']) .'" target="_blank">';
...
function check_username($username) {
	/*
	  this function checks the validity of $username. 
	  	1.	checks if the string is at least 3 characters long
	  	2.	checks if it is alpha-numeric
	  	3.	checks if it already exists in DB
	 -> Returns False if the $username is OK
	*/  
	global $conn_id;
	if( empty($conn_id) ) {
		$conn_id =  db_connect();
	}
	$username = magicSlashes(trim($username));
	$rows = 0;
	if(strlen(stripslashes($username)) < 3)
	 	return 1; // too short or empty
		
	$alpha_num = str_replace(" ", "", $username);
	if( !ctype_alnum($alpha_num))
		return 2; // invalid characters -> non-alpha-numeric
	
	$sql = "SELECT username FROM pm_users WHERE LOWER(username) = '".htmlspecialchars(strtolower($username))."'";
	$result = @mysql_query($sql, $conn_id);
	if(!$result) { 
	  	return false;
	}
...
function show_thumb($uniq_id, $t_id = 1)
{
	if(_THUMB_FROM == 1) 	//	Outsource
	{
		...
		if(($r['source_id'] == 1) || ($r['yt_thumb'] != '' && strstr($r['yt_thumb'], "http://") === false))	//	thumbnail is hosted locally
		{
			...
			elseif($r['yt_thumb'] != '' && strstr($r['yt_thumb'], "http://") === false)
			{
		...
		else
		{
			if($r['yt_thumb'] == '')
			{
				if($r['source_id'] == 3)
				{
					...
					$thumb_url =  _YOUTUBETHUB."/".$r['yt_id']."/default.jpg";
				}
				else
				{
					$thumb_url = _NOTHUMB;
...
function makevideolink($uniq_id, $video_title = '')
{
	// legacy function - keep it
	$r = array();
	...
	if (_SEOMOD == 1) 
	{
		...
		{
			$query 	= @mysql_query("SELECT video_title FROM pm_videos WHERE uniq_id = '".$uniq_id."'");
			$r 		= mysql_fetch_array($query);
			mysql_free_result($query);
		}
		$video_title_clean = sanitize_title($r['video_title']);
		
		$return = _URL .'/'. $video_title_clean .'-video_'. $uniq_id .'.html';
	} 
	else 
	{
...
function log_error($log_msg, $area, $msg_type) 
{

	if(!empty($log_msg)) 
	{
		$sql = "INSERT INTO pm_log (log_msg, area, msg_type, added) VALUES ('".$log_msg."', '".$area."', '".$msg_type."', '".time()."')";
		$result = mysql_query($sql);
	}
	return true;
}
...
function add_meta($item_id, $item_type, $meta_key, $meta_value, $check = false)
{
	if (!$item_id || !$item_type || !$meta_key)
	{
		return false;
	}
...
function update_meta($item_id, $item_type, $meta_key, $meta_value)
{
	if (!$item_id || !$item_type || !$meta_key)
	{
		return false;
	...
	if (mysql_num_rows($result) == 0)
	{
		add_meta($item_id, $item_type, $meta_key, $meta_value);
		return true; 
	}
	mysql_free_result($result);
...
function delete_meta($item_id, $item_type, $meta_key = '')
{
...
function session_save_footprint()
{
	...
	if ($dir_name == 'admin')
	{
		$ignore_pages = array('login.php', 'article_ajax.php', 'footer.php', 'header.php', 
							  'functions.php', 'img.resize.php', 'mysql_backup.php',
							  'page_ajax.php', 'styleme.php', 'upload_file.php', 'upload_image.php',
							  'vscheck.php' 
							  );
...
function get_preroll_ad($ad_id = '')
{
	...
	$rand_from = abs(rand(0, $total_available_ads - 1));
...
function serve_preroll_ad($page = '', $video_data)
{
	...
	if ($page == '' || ! in_array($page, array('index', 'detail', 'favorites')))
	{
		$page = 'detail';
	}
